...
/**
 * This is a near duplicate of {@link }, modified for use with
...
class ComparableTimSort {
    ...
    static void sort(Object[] a, int lo, int hi, Object[] work, int workBase, int workLen) {
        ...
        //二分法，直到子序列小于32时停止，如果其中遇到奇数 则子序列+1
...
//            stackSize++;
    ...
    private void mergeCollapse() {
        ...
        while (stackSize > 1) {
            // 倒数第二个元素位置
            int n = stackSize - 2;
            //三个及以上元素时，倒数第三个长度 小于 倒数第二个与倒数第一个的和
            if (n > 0 && runLen[n-1] <= runLen[n] + runLen[n+1]) {
                //倒数第三长度比倒数第一个小 则merge 倒数第三 和 倒数第二
    ...
    private static int gallopRight(Comparable<Object> key, Object[] a,
        ...
        int ofs = 1;
        int lastOfs = 0;
        if (key.compareTo(a[base]) < 0) {
            // Gallop left until a[b+hint - ofs] <= key < a[b+hint - lastOfs]
            int maxOfs = 1;
            while (ofs < maxOfs && key.compareTo(a[base - ofs]) < 0) {
                lastOfs = ofs;
                ofs = (ofs << 1) + 1;
                if (ofs <= 0)   // int overflow
                    ofs = maxOfs;
    ...
    private void mergeLo(int base1, int len1, int base2, int len2) {
        ...
        Object[] tmp = ensureCapacity(len1);//长度： len1上面一个2的幂次数 与 a.length/2 的较小者
        ...
        a[dest++] = a[cursor2++];
        if (--len2 == 0) {// 第二个数组只有一个元素，第一个也之后一个
    ...
    //取一个数下面最大的2的幂
    //return the largest power of 2 under this number
    private int getMax2Pow(int n){
        n |= n >> 1;
        n |= n >> 2;
        n |= n >> 4;
